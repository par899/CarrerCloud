...
namespace CareerCloud.gRPC.Services
{
    public class CompanyProfileService:CompanyProfileBase
    {
        ...
        public override Task<CompanyProfilePayload> ReadCompanyProfile(IdRequestCProfile request, ServerCallContext context)
        {
            CompanyProfilePoco poco = _logic.Get(Guid.Parse(request.Id));
            return new Task<CompanyProfilePayload>(
                    ()=> new CompanyProfilePayload() {
                              Id = poco.ToString(),
                              RegistrationDate = poco.RegistrationDate is null ? null : Timestamp.FromDateTime((DateTime)poco.RegistrationDate),
                        
                            CompanyWebsite = poco.ToString(),
                            ContactPhone = poco.ToString(),
                            ContactName = poco.ToString(),
